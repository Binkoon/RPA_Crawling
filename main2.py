...
# # 엑셀 로그 데이터를 저장할 리스트 (스레드 안전성을 위한 락 추가)
# excel_log_data = []
...
#     # 병렬 처리 성능 분석
#     if len(crawling_results) > 1:
#         # 순차 처리 시 예상 시간 계산 (가장 긴 선사 시간 기준)
#         max_carrier_time = max([result.get('duration', 0) for _, result in crawling_results])
#         estimated_sequential_time = sum([result.get('duration', 0) for _, result in crawling_results])
#         time_saved = estimated_sequential_time - total_duration
#         time_saved_percentage = (time_saved / estimated_sequential_time * 100) if estimated_sequential_time > 0 else 0
        
#         print(f"\n병렬 처리 성능 분석:")
#         print(f"가장 긴 선사 처리 시간: {max_carrier_time:.2f}초")
#         print(f"순차 처리 시 예상 시간: {estimated_sequential_time:.2f}초")
#         print(f"시간 단축: {time_saved:.2f}초 ({time_saved_percentage:.1f}%)")
