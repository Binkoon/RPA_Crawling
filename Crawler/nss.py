...
class NSS_Crawling(ParentsClass):
        ...
        try:
            ...
            for vessel_name in self.vessel_name_list:
                try:
                    ...
                    self.end_vessel_tracking(vessel_name, success=True)
                    vessel_duration = self.get_vessel_duration(vessel_name)
                    self.logger.error(f"선박 {vessel_name} 크롤링 완료 (소요시간: {vessel_duration:.2f}초)")
                    continue
    ...
    def retry_failed_vessels(self, failed_vessels):
        ...
        self.logger.info(f"=== NSS 실패한 선박 재시도 시작 ===")
        ...
        # 재시도 결과 요약
        self.logger.info("="*60)
        self.logger.info("NSS 재시도 결과 요약")
        self.logger.info("="*60)
        ...
        return {
            ...
            'note': f'NSS 재시도 완료 - 성공: {retry_success_count}개, 실패: {retry_fail_count}개'
        }
